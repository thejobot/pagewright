/**
 * Publish or save a Pagewright-edited page (and any accompanying assets)
 * to the configured GitHub repo as a single atomic commit + PR.
 *
 * POST /publish/commit
 *
 * Body shapes:
 ...
 *       pagewright/<slug>-<shortSha>.
 *
 *   { mode: "edit", path, sha, html, assets? }
 *     → updates the file at path on branch pagewright/edit-<safePath>-<shortSha>.
 *
 *   assets: [{ path, base64 }]
...
export async function onRequestPost(context) {
  ...
  if (mode === "edit") {
    ...
    prTitle = `Pagewright: edit ${htmlPath}`;
  } else {
  ...
  try {
    ...
    const branch = `pagewright/${branchBase}-${baseSha.slice(0, 7)}`;
